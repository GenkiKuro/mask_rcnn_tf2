...
class MaskRCNN(object):
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        with h5py.File(filepath, mode='r') as f:
            ...
            #   for layer in layers:
            #     if layer.name == 'mrcnn_bbox_fc':  # Check if it's the problematic layer
            #         weights_kernel  = f['mrcnn_bbox_fc']['mrcnn_bbox_fc']['kernel:0'][:]
            #         weights_bias = f['mrcnn_bbox_fc']['mrcnn_bbox_fc']['bias:0'][:]
            #         # Reshape the weights to match the expected shape (1024, 28)
            #         # reshaped_weights = weights.reshape((1024, 28))
            #         # Set the weights for the layer
            #         layer.set_weights([weights_kernel, weights_bias])
            #     else:
            #         # Load weights for other layers as usual
